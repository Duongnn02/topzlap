define("discourse-common/lib/deprecated", ["exports"], function (_exports) {
  ...
  _exports.withSilencedDeprecations = withSilencedDeprecations;
  _exports.withSilencedDeprecationsAsync = withSilencedDeprecationsAsync;
  0; //eaimeta@70e063a35619d71feaimeta@70e063a35619d71f
  const handlers = [];
  const disabledDeprecations = new Set();
  const emberCliDeprecationWorkflows = window.deprecationWorkflow?.config?.workflow;
  ...
  function deprecated(msg) {
    ...
    const matchedWorkflow = emberCliDeprecationWorkflows?.find(w => w.matchId === id);
   ...
   */
  function registerDeprecationHandler(callback) {
   ...
   */
  function withSilencedDeprecations(deprecationIds, callback) {
   ...
   */
  async function withSilencedDeprecationsAsync(deprecationIds, callback) {
    const idArray = [].concat(deprecationIds);
    try {
      idArray.forEach(id => disabledDeprecations.add(id));
