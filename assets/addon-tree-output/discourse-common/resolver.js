define("discourse-common/resolver", ["exports", "@ember/string", "discourse-common/lib/deprecated", "discourse-common/lib/helpers", "discourse-common/lib/suffix-trie", "ember-resolver", "discourse-common/lib/discourse-template-map"], function (_exports, _string, _deprecated, _helpers, _suffixTrie, _emberResolver, _discourseTemplateMap) {
  "use strict";
  ...
  0; //eaimeta@70e063a35619d71f0,"@ember/string",0,"discourse-common/lib/deprecated",0,"discourse-common/lib/helpers",0,"discourse-common/lib/suffix-trie",0,"ember-resolver",0,"discourse-common/lib/discourse-template-map"eaimeta@70e063a35619d71f
  let _options = {};
  ...
  function lookupModuleBySuffix(suffix) {
    if (!moduleSuffixTrie) {
      moduleSuffixTrie = new _suffixTrie.default("/");
      const searchPaths = ["discourse/",
      // Includes themes/plugins
      "discourse-common/", "select-kit/", "admin/", "wizard/", "truth-helpers/"];
  ...
  function buildResolver(baseName) {
    return class extends _emberResolver.default {
      ...
      _normalize(fullName) {
        ...
        if (deprecationInfo) {
          if (!deprecationInfo.silent) {
            (0, _deprecated.default)(`"${fullName}" is deprecated, use "${deprecationInfo.newName}" instead`, {
        ...
        if (split.length > 1 && (type === "controller" || type === "route" || type === "template")) {
          ...
          if (split[1].includes(".") || split[1].includes("/")) {
            ...
            const dashed = (0, _string.dasherize)(split[1].replace(/[\.\/]/g, "-"));
            const adminBase = `admin/${type}s/`;
            const wizardBase = `wizard/${type}s/`;
            if (lookupModuleBySuffix(`${type}s/${dashed}`) || requirejs.entries[adminBase + dashed] || requirejs.entries[adminBase + dashed.replace(/^admin[-]/, "")] || requirejs.entries[adminBase + dashed.replace(/^admin[-]/, "").replace(/-/g, "_")] || requirejs.entries[wizardBase + dashed] || requirejs.entries[wizardBase + dashed.replace(/^wizard[-]/, "")] || requirejs.entries[wizardBase + dashed.replace(/^wizard[-]/, "").replace(/-/g, "_")]) {
      ...
      chooseModuleName(moduleName, parsedName) {
        ...
        for (let name of variants) {
          ...
          resolved = lookupModuleBySuffix((0, _string.dasherize)(suffix));
          if (resolved) {
            return resolved;
          }
        }
      ...
      resolveHelper(parsedName) {
        return (0, _helpers.findHelper)(parsedName.fullNameWithoutType);
      }
      ...
      resolveTemplate(parsedName) {
        return this.findMobileTemplate(parsedName) || this.findTemplate(parsedName) || this.findAdminTemplate(parsedName) || this.findWizardTemplate(parsedName) || this.findLoadingTemplate(parsedName) || this.findConnectorTemplate(parsedName) || this.discourseTemplateModule("not_found");
      }
      ...
      discourseTemplateModule(name) {
        const resolvedName = _discourseTemplateMap.default.resolve(name);
        if (resolvedName) {
      ...
      findTemplate(parsedName, prefix) {
        ...
        const withoutType = parsedName.fullNameWithoutType,
          underscored = (0, _string.decamelize)(withoutType).replace(/-/g, "_"),
      ...
      findWizardTemplate(parsedName) {
        ...
        if (namespaced) {
          let wizardParsedName = this.parseName(`template:wizard/templates/${namespaced}`);
