define("pretty-text/engines/discourse-markdown-it", ["exports", "pretty-text/allow-lister", "discourse-common/lib/object", "discourse-common/lib/deprecated", "pretty-text/guid", "pretty-text/sanitizer"], function (_exports, _allowLister, _object, _deprecated, _guid, _sanitizer) {
  "use strict";
  ...
  0; //eaimeta@70e063a35619d71f0,"pretty-text/allow-lister",0,"discourse-common/lib/object",0,"discourse-common/lib/deprecated",0,"pretty-text/guid",0,"pretty-text/sanitizer"eaimeta@70e063a35619d71f
  ...
  function createHelper(featureName, opts, optionCallbacks, pluginCallbacks, customMarkdownCookFnCallbacks, getOptions, allowListed) {
    let helper = {};
  ...
  function buildCustomMarkdownCookFunction(engineOpts, defaultEngineOpts) {
    ...
    newOpts.discourse = {};
    const featureConfig = (0, _object.cloneJSON)(defaultEngineOpts.discourse.features);
    ...
    newOpts.engine = createMarkdownItEngineWithOpts(markdownitOpts, engineOpts.markdownItRules);
  ...
  function setupMarkdownEngine(opts, featureConfig) {
    const quotation_marks = opts.discourse.limitedSiteSettings.markdownTypographerQuotationMarks;
    if (quotation_marks) {
      opts.engine.options.quotes = quotation_marks.split("|");
    }
    opts.engine.linkify.tlds((opts.discourse.limitedSiteSettings.markdownLinkifyTlds || "").split("|"));
    ...
    opts.pluginCallbacks.forEach(_ref => {
      let [feature, callback] = _ref;
    ...
    if (!opts.discourse.sanitizer || !opts.sanitizer) {
      const allowLister = new _allowLister.default(opts.discourse);
      opts.allowListed.forEach(_ref2 => {
        let [feature, info] = _ref2;
        allowLister.allowListFeature(feature, info);
      });
      opts.sanitizer = opts.discourse.sanitizer = !!opts.discourse.sanitize ? a => (0, _sanitizer.sanitize)(a, allowLister) : a => a;
    }
  }
  ...
  function setup(opts, siteSettings, state) {
    ...
    features.sort((a, b) => a.priority - b.priority).forEach(markdownFeature => {
      markdownFeature.setup(createHelper(markdownFeature.id, opts, optionCallbacks, pluginCallbacks, customMarkdownCookFnCallbacks, getOptions, allowListed));
    });
    Object.entries(state.allowListed || {}).forEach(entry => {
      allowListed.push(entry);
    });
    optionCallbacks.forEach(_ref3 => {
      let [, callback] = _ref3;
    ...
    opts.discourse.limitedSiteSettings = {
      ...
      enableExperimentalHashtagAutocomplete: siteSettings.enable_experimental_hashtag_autocomplete
    };
    const markdownitOpts = {
    ...
    opts.engine = createMarkdownItEngineWithOpts(markdownitOpts, opts.discourse.markdownItRules);
    opts.pluginCallbacks = pluginCallbacks;
    opts.allowListed = allowListed;
    setupMarkdownEngine(opts, opts.discourse.features);
    customMarkdownCookFnCallbacks.forEach(_ref4 => {
      let [, callback] = _ref4;
      callback(opts, (engineOpts, afterBuild) => afterBuild(buildCustomMarkdownCookFunction(engineOpts, opts)));
    });
  }
