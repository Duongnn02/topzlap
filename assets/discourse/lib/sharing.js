define("discourse/lib/sharing", ["exports"], function (_exports) {
  "use strict";
  ...
  var _default = {
    ...
    shareSource(source, data) {
      if (source.clickHandler) {
        ...
        const stringOptions = Object.keys(options).map(k => `${k}=${options[k]}`).join(",");
    ...
    activeSources() {
      let linksSetting = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "";
      let privateContext = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
      const sources = linksSetting.split("|").concat(_customSharingIds).map(s => _sources[s]).compact();
      return privateContext ? sources.filter(s => s.showInPrivateContext) : sources;
    },
    _reset() {
      _sources = {};
      _customSharingIds = [];
