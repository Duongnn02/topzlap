define("discourse/lib/transform-post", ["exports", "I18n", "@ember/utils", "discourse/lib/url", "discourse-common/lib/get-url"], function (_exports, _I18n, _utils, _url, _getUrl) {
  "use strict";

  Object.defineProperty(_exports, "__esModule", {
    value: true
  });
  _exports.default = transformPost;
  _exports.includeAttributes = includeAttributes;
  _exports.transformBasicPost = transformBasicPost;
  0; //eaimeta@70e063a35619d71f0,"I18n",0,"@ember/utils",0,"discourse/lib/url",0,"discourse-common/lib/get-url"eaimeta@70e063a35619d71f
  ...
  function transformBasicPost(post) {
    ...
    const postAtts = {
      ...
      canFlag: !post.get("topic.deleted") && !(0, _utils.isEmpty)(post.get("flagsAvailable")),
  ...
  function transformPost(currentUser, site, post, prevPost, nextPost) {
    ...
    const filteredRepliesPostNumber = topic.get("postStream.filterRepliesToPostNumber");
    ...
    postAtts.canPublishPage = !!details.can_publish_page && post.post_number === 1;
    postAtts.isWarning = topic.is_warning;
    postAtts.links = post.get("internalLinks");
    postAtts.replyDirectlyBelow = nextPost && nextPost.reply_to_post_number === post.post_number && post.post_number !== filteredRepliesPostNumber;
    postAtts.replyDirectlyAbove = prevPost && post.id !== filteredUpwardsPostID && post.reply_to_post_number === prevPost.post_number;
    postAtts.linkCounts = post.link_counts;
    postAtts.actionCode = post.action_code;
    postAtts.actionCodeWho = post.action_code_who;
    postAtts.actionCodePath = (0, _getUrl.default)(post.action_code_path || `/t/${topic.id}`);
    ...
    const showTopicMap = _additionalAttributes.includes("topicMap") && post.post_number === 1 || showPMMap || post.post_number === 1 && topic.archetype === "regular" && topic.posts_count > 1;
    if (showTopicMap) {
    ...
    if (post.actions_summary) {
      postAtts.actionsSummary = post.actions_summary.filter(a => {
        ...
        return {
          ...
          action,
          canUndo: a.can_undo,
          description: _I18n.default.t(`post.actions.by_you.${action}`)
        };
      });
    }
    ...
    } else if (!currentUser || topic.archived && topic.user_id !== currentUser.id) {
    ...
    if (postAtts.post_number === 1) {
      ...
      postAtts.canPermanentlyDelete = postAtts.isDeleted && details.can_permanently_delete;

      // Show a "Flag to delete" message if not staff and you can't
      // otherwise delete it.
      postAtts.showFlagDelete = !postAtts.canDelete && postAtts.yours && postAtts.canFlag && currentUser && !currentUser.staff;
    } else {
      ...
      postAtts.canPermanentlyDelete = postAtts.isDeleted && post.can_permanently_delete;
    }
    _additionalAttributes.forEach(a => postAtts[a] = post[a]);
    return postAtts;
  }
