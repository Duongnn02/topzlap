define("discourse/lib/desktop-notifications", ["exports", "discourse/lib/url", "I18n", "discourse/lib/key-value-store", "rsvp", "discourse/models/site", "discourse/models/user", "discourse/lib/utilities", "discourse-common/lib/later"], function (_exports, _url, _I18n, _keyValueStore, _rsvp, _site, _user, _utilities, _later) {
  "use strict";
  ...
  0; //eaimeta@70e063a35619d71f0,"discourse/lib/url",0,"I18n",0,"discourse/lib/key-value-store",0,"rsvp",0,"discourse/models/site",0,"discourse/models/user",0,"discourse/lib/utilities",0,"discourse-common/lib/later"eaimeta@70e063a35619d71f
  ...
  const keyValueStore = new _keyValueStore.default(context);
  ...
  function confirmNotification(siteSettings) {
    const notification = new Notification(_I18n.default.t("notifications.popup.confirm_title", {
  ...
  async function onNotification(data, siteSettings, user) {
    ...
    const notificationTitle = data.translated_title || _I18n.default.t(i18nKey(data.notification_type), {
      site_title: siteSettings.title,
      topic: data.topic_title,
      username: (0, _utilities.formatUsername)(data.username)
    });
    const notificationBody = data.excerpt;
    const notificationIcon = siteSettings.site_logo_small_url || siteSettings.site_logo_url;
    const notificationTag = "discourse-notification-" + siteSettings.title + "-" + data.topic_id;
    ...
    desktopNotificationHandlers.forEach(handler => handler(data, siteSettings, user));
  }
