define("discourse/lib/uppy-chunked-upload", ["exports", "rsvp", "@uppy/utils/lib/delay", "@uppy/utils/lib/AbortController"], function (_exports, _rsvp, _delay, _AbortController) {
  "use strict";
  ...
  0; //eaimeta@70e063a35619d71f0,"rsvp",0,"@uppy/utils/lib/delay",0,"@uppy/utils/lib/AbortController"eaimeta@70e063a35619d71f
  const MB = 1024 * 1024;
  ...
  class UppyChunkedUpload {
    ...
    _retryable(_ref) {
      ...
      const doAttempt = retryAttempt => attempt().catch(err => {
        ...
        if (this._shouldRetry(err) && retryAttempt < retryDelays.length) {
          return (0, _delay.default)(retryDelays[retryAttempt], {
            signal
    ...
    _uploadChunk(index) {
      ...
      return this._uploadChunkBytes(index, this.options.url, this.options.headers);
    }
    _onChunkProgress(index, sent) {
      this.chunkState[index].bytesUploaded = parseInt(sent, 10);
      const totalUploaded = this.chunkState.reduce((total, chunk) => total + chunk.bytesUploaded, 0);
    ...
    _buildFormData(currentChunkNumber, body) {
      const uniqueIdentifier = this.file.data.size + "-" + this.file.data.name.replace(/[^0-9a-zA-Z_-]/gim, "");
  ...
  _exports.default = UppyChunkedUpload;
});