define("discourse/lib/register-topic-footer-button", ["exports", "I18n", "@ember/object"], function (_exports, _I18n, _object) {
  "use strict";
  ...
  0; //eaimeta@70e063a35619d71f0,"I18n",0,"@ember/object"eaimeta@70e063a35619d71f
  ...
  function registerTopicFooterButton(button) {
    ...
    if (!normalizedButton.icon && !normalizedButton.title && !normalizedButton.translatedTitle) {
      throw new Error(`Attempted to register a topic button: ${button.id} with no icon or title.`);
    }
    _topicFooterButtons[normalizedButton.id] = normalizedButton;
  }
  function getTopicFooterButtons() {
    const dependentKeys = [].concat(...Object.values(_topicFooterButtons).map(tfb => tfb.dependentKeys).filter(x => x));
    return (0, _object.computed)(...dependentKeys, {
      get() {
        ...
        return Object.values(_topicFooterButtons).filter(button => _compute(button, "displayed")).map(button => {
