define("discourse/lib/keyboard-shortcuts", ["exports", "discourse-common/utils/decorators", "discourse-common/lib/debounce", "@ember/runloop", "discourse-common/lib/later", "discourse/lib/topic-list-tracker", "discourse/models/composer", "discourse/lib/url", "discourse-common/utils/dom-utils", "discourse-common/config/environment", "discourse/lib/ajax", "discourse/lib/offset-calculator", "discourse-common/lib/helpers"], function (_exports, _decorators, _debounce, _runloop, _later, _topicListTracker, _composer, _url, _domUtils, _environment, _ajax, _offsetCalculator, _helpers) {
  "use strict";

  Object.defineProperty(_exports, "__esModule", {
    value: true
  });
  _exports.clearExtraKeyboardShortcutHelp = clearExtraKeyboardShortcutHelp;
  _exports.extraKeyboardShortcutsHelp = _exports.default = void 0;
  var _obj;
  0; //eaimeta@70e063a35619d71f0,"discourse-common/utils/decorators",0,"discourse-common/lib/debounce",0,"@ember/runloop",0,"discourse-common/lib/later",0,"discourse/lib/topic-list-tracker",0,"discourse/models/composer",0,"discourse/lib/url",0,"discourse-common/utils/dom-utils",0,"discourse-common/config/environment",0,"discourse/lib/ajax",0,"discourse/lib/offset-calculator",0,"discourse-common/lib/helpers"eaimeta@70e063a35619d71f
  ...
  _exports.extraKeyboardShortcutsHelp = extraKeyboardShortcutsHelp;
  function addExtraKeyboardShortcutHelp(help) {
    const category = help.category;
    if (extraKeyboardShortcutsHelp[category]) {
      extraKeyboardShortcutsHelp[category] = extraKeyboardShortcutsHelp[category].concat([help]);
    } else {
      extraKeyboardShortcutsHelp[category] = [help];
    }
  }
  function clearExtraKeyboardShortcutHelp() {
    _exports.extraKeyboardShortcutsHelp = extraKeyboardShortcutsHelp = {};
  }
  const DEFAULT_BINDINGS = {
    ...
    d: {
      postAction: "deletePostWithConfirmation"
    },
    e: {
      handler: "editPost"
    },
  ...
  var _default = (_obj = {
    ...
    bindKey(key) {
      let binding = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
      if (this.isTornDown()) {
        return;
      }
      if (!binding) {
    ...
    quoteReply() {
      ...
      (0, _later.default)(() => document.querySelector(".d-editor .quote")?.click(), 500);
      return false;
    },
    editPost() {
    ...
    goToNextTopic() {
      (0, _topicListTracker.nextTopicUrl)().then(url => {
        if (url) {
    ...
    goToPreviousTopic() {
      (0, _topicListTracker.previousTopicUrl)().then(url => {
        if (url) {
          _url.default.routeTo(url);
        }
      });
    ...
    setTrackingToMuted() {
      (0, _runloop.throttle)(this, "_setTracking", 0, _environment.INPUT_DELAY, true);
    },
    setTrackingToRegular() {
      (0, _runloop.throttle)(this, "_setTracking", 1, _environment.INPUT_DELAY, true);
    },
    setTrackingToTracking() {
      (0, _runloop.throttle)(this, "_setTracking", 2, _environment.INPUT_DELAY, true);
    },
    setTrackingToWatching() {
      (0, _runloop.throttle)(this, "_setTracking", 3, _environment.INPUT_DELAY, true);
    },
    ...
    sendToSelectedPost(action, elem) {
      ...
      const selectedPost = elem || document.querySelector(".topic-post.selected article.boxed");
      let selectedPostId;
      if (selectedPost) {
    ...
    _bindToPath(path, key) {
      this.keyTrapper.bind(key, () => _url.default.routeTo(path));
    },
    ...
    _moveSelection(direction) {
      ...
      const now = +new Date();
      const fast = this._lastMoveTime && now - this._lastMoveTime < 1.5 * animationDuration;
      ...
      let selected = articles.find(element => element.classList.contains("selected"));
      if (!selected) {
        selected = articles.find(element => element.dataset.islastviewedtopic === "true");
      }
      ...
      if (selected && !fast) {
        ...
        if (rect.bottom < (0, _offsetCalculator.headerOffset)() || rect.top > window.innerHeight) {
          selected = null;
        }
      }
      ...
      if (!selected) {
        ...
        selected = articles.find(article => direction > 0 ? article.getBoundingClientRect().top >= offset : article.getBoundingClientRect().bottom >= offset);
        if (!selected) {
      ...
      if (!fast && direction !== 0 && article) {
        ...
        const beginArticle = article.classList.contains("topic-post") && article.querySelector("#post_1") ? 0 : _domUtils.default.offset(article).top;
        const endArticle = _domUtils.default.offset(article).top + article.offsetHeight;
        const beginScreen = window.scrollY;
        const endScreen = beginScreen + window.innerHeight;
        if (direction < 0 && beginScreen > beginArticle) {
          return this._scrollTo(Math.max(beginScreen - window.innerHeight + 3 * (0, _offsetCalculator.headerOffset)(),
          // page up
          beginArticle - (0, _offsetCalculator.headerOffset)() // beginning of article
          ));
        } else if (direction > 0 && endScreen < endArticle - (0, _offsetCalculator.headerOffset)()) {
          return this._scrollTo(Math.min(endScreen - 3 * (0, _offsetCalculator.headerOffset)(),
          // page down
      ...
      const articleTop = _domUtils.default.offset(article).top,
    ...
    categoriesTopicsList() {
      switch (this.siteSettings.desktop_category_page_style) {
        ...
        case "categories_and_latest_topics_created_date":
          return document.querySelectorAll(".latest-topic-list .latest-topic-list-item");
        case "categories_and_top_topics":
          return document.querySelectorAll(".top-topic-list .latest-topic-list-item");
        default:
          return [];
      }
    },
    _findArticles() {
      ...
      if (document.querySelector(".posts-wrapper")) {
        return document.querySelectorAll(".posts-wrapper .topic-post, .topic-list tbody tr");
    ...
    webviewKeyboardBack() {
      if ((0, _helpers.helperContext)().capabilities.isAppWebview) {
        window.history.back();
      }
    },
