define("discourse/lib/to-markdown", ["exports"], function (_exports) {
  "use strict";
  ...
  _exports.Tag = void 0;
  _exports.addBlockDecorateCallback = addBlockDecorateCallback;
  _exports.addTagDecorateCallback = addTagDecorateCallback;
  _exports.addTextDecorateCallback = addTextDecorateCallback;
  _exports.clearBlockDecorateCallbacks = clearBlockDecorateCallbacks;
  _exports.clearTagDecorateCallbacks = clearTagDecorateCallbacks;
  _exports.clearTextDecorateCallbacks = clearTextDecorateCallbacks;
  _exports.default = toMarkdown;
  0; //eaimeta@70e063a35619d71feaimeta@70e063a35619d71f
  const MSO_LIST_CLASSES = ["MsoListParagraphCxSpFirst", "MsoListParagraphCxSpMiddle", "MsoListParagraphCxSpLast"];
   ...
   * ```
   */
  function addTagDecorateCallback(callback) {
    tagDecorateCallbacks.push(callback);
  }
  function clearTagDecorateCallbacks() {
   ...
   * ```
   */
  function addBlockDecorateCallback(callback) {
    blockDecorateCallbacks.push(callback);
  }
  function clearBlockDecorateCallbacks() {
   ...
   * ```
   */
  function addTextDecorateCallback(callback) {
    textDecorateCallbacks.push(callback);
  }
  function clearTextDecorateCallbacks() {
  ...
  class Tag {
    ...
    static trimmable() {
      return [...Tag.blocks(), ...Tag.headings(), ...Tag.slices(), "aside", "li", "td", "th", "br", "hr", "blockquote", "table", "ol", "tr", "ul"];
    }
    static allowedTags() {
      return ["ins", "del", "small", "big", "kbd", "ruby", "rt", "rb", "rp", "mark"];
    }
    ...
    static aside() {
      return class extends Tag.block("aside") {
        ...
        toMarkdown() {
          ...
          const blockquote = this.element.children.find(child => child.name === "blockquote");
          if (!blockquote) {
            return super.toMarkdown();
          }
          ...
          const prefix = username && post && topic ? `[quote="${username}, post:${post}, topic:${topic}"]` : "[quote]";
          return `\n${prefix}\n${text}\n[/quote]\n`;
        }
      };
    }
    ...
    static link() {
      return class extends Tag.named("a") {
        ...
        decorate(text) {
          ...
          let img;
          if (["lightbox", "d-lazyload"].includes(attr.class) && (img = (e.children || []).find(c => c.name === "img"))) {
            let href = attr.href;
      ...
      return class extends Tag.slice("li", "\n") {
        decorate(text) {
          ...
          if (MSO_LIST_CLASSES.includes(attrs.class)) {
            try {
              const level = parseInt(attrs.style.match(/level./)[0].replace("level", ""), 10);
    ...
    static table() {
      return class extends Tag.block("table") {
        ...
        decorate(text) {
          ...
          this.isValid = this.isValid && rows.length > 1 && pipeCount > 2 && rows.reduce((a, c) => a && this.countPipes(c) <= pipeCount); // Unsupported table format for Markdown conversion
  ...
  function tagByName(name) {
    if (!tagsMap) {
      tagsMap = new Map();
      const allTags = [...Tag.blocks().map(b => Tag.block(b)), ...Tag.headings().map((h, i) => Tag.heading(h, i + 1)), ...Tag.slices().map(s => Tag.slice(s, "\n")), ...Tag.emphases().map(e => Tag.emphasis(e[0], e[1])), ...Tag.allowedTags().map(t => Tag.allowedTag(t)), Tag.aside(), Tag.cell("td"), Tag.cell("th"), Tag.replace("br", "\n"), Tag.replace("hr", "\n---\n"), Tag.replace("head", ""), Tag.li(), Tag.link(), Tag.image(), Tag.code(), Tag.blockquote(), Tag.table(), Tag.tr(), Tag.ol(), Tag.list("ul"), Tag.span()];
  ...
  class Element {
    ...
    static parse(elements) {
      ...
      if (elements) {
        ...
        for (let i = 0; i < elements.length; i++) {
          ...
          result.push(Element.toMarkdown(elements[i], parent, prev, next, metadata));
        }
        return result.join("");
      }
      return "";
    ...
    text() {
      ...
      textDecorateCallbacks.forEach(callback => {
        const result = callback.call(this, text, this.next, this.previous, this.metadata);
        if (typeof result !== "undefined") {
          text = result;
        }
      });
