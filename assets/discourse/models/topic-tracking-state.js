define("discourse/models/topic-tracking-state", ["exports", "@ember/object", "discourse-common/utils/decorators", "discourse/models/category", "discourse-common/lib/object", "discourse/lib/url", "discourse/lib/notification-levels", "discourse/lib/preload-store", "discourse/models/user", "discourse/models/site", "@ember/utils"], function (_exports, _object, _decorators, _category, _object2, _url, _notificationLevels, _preloadStore, _user, _site, _utils) {
  ...
  var _dec, _obj;
  0; //eaimeta@70e063a35619d71f0,"@ember/object",0,"discourse-common/utils/decorators",0,"discourse/models/category",0,"discourse-common/lib/object",0,"discourse/lib/url",0,"discourse/lib/notification-levels",0,"discourse/lib/preload-store",0,"discourse/models/user",0,"discourse/models/site",0,"@ember/utils"eaimeta@70e063a35619d71f
  ...
  function hasMutedTags(topicTags, mutedTags, siteSettings) {
    ...
    return siteSettings.remove_muted_tags_from_latest === "always" && topicTags.any(topicTag => mutedTags.includes(topicTag)) || siteSettings.remove_muted_tags_from_latest === "only_muted" && topicTags.every(topicTag => mutedTags.includes(topicTag));
  }
  const TopicTrackingState = _object.default.extend((_dec = (0, _decorators.default)("incomingCount"), (_obj = {
    messageCount: 0,
    init() {
    ...
    willDestroy() {
      ...
      if (this.currentUser) {
        ...
        this.messageBus.unsubscribe(`/unread/${this.currentUser.id}`, this._processChannelPayload);
      }
    ...
    establishChannels(meta) {
      ...
      this.messageBus.subscribe("/latest", this._processChannelPayload, meta["/latest"] || messageBusDefaultNewMessageId);
      if (this.currentUser) {
        this.messageBus.subscribe("/new", this._processChannelPayload, meta["/new"] || messageBusDefaultNewMessageId);
        this.messageBus.subscribe(`/unread`, this._processChannelPayload, meta["/unread"] || messageBusDefaultNewMessageId);
        this.messageBus.subscribe(`/unread/${this.currentUser.id}`, this._processChannelPayload, meta[`/unread/${this.currentUser.id}`] || messageBusDefaultNewMessageId);
      }
      this.messageBus.subscribe("/delete", this.onDeleteMessage, meta["/delete"] || messageBusDefaultNewMessageId);
      this.messageBus.subscribe("/recover", this.onRecoverMessage, meta["/recover"] || messageBusDefaultNewMessageId);
      this.messageBus.subscribe("/destroy", this.onDestroyMessage, meta["/destroy"] || messageBusDefaultNewMessageId);
    },
    onDeleteMessage(msg) {
    ...
    onDestroyMessage(msg) {
      this.incrementMessageCount();
      const currentRoute = _url.default.router.currentRoute.parent;
    ...
    pruneOldMutedAndUnmutedTopics() {
      const now = Date.now();
      let mutedTopics = this.mutedTopics().filter(mutedTopic => now - mutedTopic.createdAt < 60000);
      let unmutedTopics = this.unmutedTopics().filter(unmutedTopic => now - unmutedTopic.createdAt < 60000);
    ...
    notifyIncoming(data) {
      ...
      if (filterCategory && filterCategory.get("id") !== categoryId) {
        const category = categoryId && _category.default.findById(categoryId);
      ...
      if (filter === "categories" && data.message_type === "latest" && !_site.default.current().mobileView && (this.siteSettings.desktop_category_page_style === "categories_and_latest_topics" || this.siteSettings.desktop_category_page_style === "categories_and_latest_topics_created_date")) {
        this._addIncoming(data.topic_id);
      }
    ...
    getSubCategoryIds(categoryId) {
      ...
      const categories = _category.default.list();
    ...
    countCategoryByState(_ref) {
      ...
      const subcategoryIds = noSubcategories ? new Set([categoryId]) : this.getSubCategoryIds(categoryId);
      ...
      return Array.from(this.states.values()).filter(topic => {
        ...
        if (categoryId && topic.is_category_topic && categoryId !== topic.category_id) {
          return false;
        }
    ...
    countCategory(category_id, tagId) {
      ...
      for (let topic of this.states.values()) {
        if (topic.category_id === category_id && !topic.deleted && (!tagId || topic.tags?.includes(tagId))) {
    ...
    lookupCount() {
      ...
      let categoryId = category ? (0, _object.get)(category, "id") : null;
      if (type === "new") {
    ...
    _fixDelayedServerState(list, filter) {
      for (let index = list.topics.length - 1; index >= 0; index--) {
        ...
        if (state && state.last_read_post_number > 0 && (topic.last_read_post_number === 0 || !this.currentUser?.new_new_view_enabled)) {
          if (filter === "new") {
    ...
    _shouldCompensateState(list, filter, queryParams) {
      let shouldCompensate = (filter === "new" || filter === "unread") && !list.more_topics_url;
    ...
    _processChannelPayload(data) {
      ...
      if (["new_topic", "latest"].includes(data.message_type)) {
        const mutedCategoryIds = _user.default.currentProp("muted_category_ids")?.concat(_user.default.currentProp("indirectly_muted_category_ids"));
        if (mutedCategoryIds && mutedCategoryIds.includes(data.payload.category_id) && !this.isUnmutedTopic(data.topic_id)) {
          return;
        }
      }
      if (["new_topic", "latest"].includes(data.message_type)) {
        const mutedTags = _user.default.currentProp("muted_tags");
      ...
      if (["new_topic", "unread", "read"].includes(data.message_type)) {
        this.notifyIncoming(data);
        if (!(0, _object2.deepEqual)(old, data.payload)) {
          ...
          if (old) {
            payload = (0, _object2.deepMerge)(old, data.payload);
          }
          if (data.message_type === "unread") {
            ...
            if (payload.notification_level === undefined) {
              ...
              payload.notification_level = _notificationLevels.NotificationLevels.TRACKING;
            }
          }
    ...
    _trackedTopics() {
      ...
      return Array.from(this.states.values()).map(topic => {
  ...
  }, (_applyDecoratedDescriptor(_obj, "onDeleteMessage", [_decorators.bind], Object.getOwnPropertyDescriptor(_obj, "onDeleteMessage"), _obj), _applyDecoratedDescriptor(_obj, "onRecoverMessage", [_decorators.bind], Object.getOwnPropertyDescriptor(_obj, "onRecoverMessage"), _obj), _applyDecoratedDescriptor(_obj, "onDestroyMessage", [_decorators.bind], Object.getOwnPropertyDescriptor(_obj, "onDestroyMessage"), _obj), _applyDecoratedDescriptor(_obj, "hasIncoming", [_dec], Object.getOwnPropertyDescriptor(_obj, "hasIncoming"), _obj), _applyDecoratedDescriptor(_obj, "_newStateFromListTopic", [_decorators.bind], Object.getOwnPropertyDescriptor(_obj, "_newStateFromListTopic"), _obj), _applyDecoratedDescriptor(_obj, "_processChannelPayload", [_decorators.bind], Object.getOwnPropertyDescriptor(_obj, "_processChannelPayload"), _obj)), _obj)));
  function startTracking(tracking) {
    _preloadStore.default.getAndRemove("topicTrackingStates").then(data => tracking.loadStates(data));
    _preloadStore.default.getAndRemove("topicTrackingStateMeta").then(meta => tracking.establishChannels(meta));
  }
