define("discourse/services/store", ["exports", "@ember/service", "@ember/object", "rsvp", "discourse/models/rest", "discourse/models/result-set", "discourse/lib/ajax", "discourse-common/lib/get-owner", "@ember/string"], function (_exports, _service, _object, _rsvp, _rest, _resultSet, _ajax, _getOwner, _string) {
  "use strict";
  ...
  0; //eaimeta@70e063a35619d71f0,"@ember/service",0,"@ember/object",0,"rsvp",0,"discourse/models/rest",0,"discourse/models/result-set",0,"discourse/lib/ajax",0,"discourse-common/lib/get-owner",0,"@ember/string"eaimeta@70e063a35619d71f
  let _identityMap;
  ...
  var _default = _service.default.extend({
    ...
    findFiltered(type, findArgs) {
      return this.adapterFor(type).find(this, type, findArgs).then(result => this._build(type, result));
    },
    ...
    _updateStale(stale, hydrated, primaryKey) {
      ...
      hydrated.set("content", hydrated.get("content").map(item => {
        ...
        if (staleItem) {
          for (const [key, value] of Object.entries(Object.getOwnPropertyDescriptors(staleItem))) {
    ...
    refreshResults(resultSet, type, url) {
      ...
      return (0, _ajax.ajax)(url).then(result => {
        const typeName = (0, _string.underscore)(this.pluralize(adapter.apiNameFor(type)));
    ...
    _resultSet(type, result, findArgs) {
      ...
      return _resultSet.default.create(createArgs);
    },
    _build(type, obj) {
      const adapter = this.adapterFor(type);
      obj.store = this;
      ...
      obj.topicTrackingState = this.register.lookup("service:topic-tracking-state");
      obj.keyValueStore = this.register.lookup("service:key-value-store");
      const klass = this.register.lookupFactory("model:" + type) || _rest.default;
    ...
    _hydrateEmbedded(type, obj, root) {
      ...
      Object.keys(obj).forEach(k => {
        ...
        if (m) {
          const subType = m[1];
          if (m[2]) {
            const hydrated = obj[k].map(id => this._lookupSubType(subType, type, id, root));
    ...
    _hydrate(type, obj, root) {
      ...
      if (!id) {
        throw new Error(`Can't hydrate ${type} without primaryKey: \`${adapter.primaryKey}\``);
      }
      root = root || obj;
