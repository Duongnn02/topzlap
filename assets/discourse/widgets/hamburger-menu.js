define("discourse/widgets/hamburger-menu", ["exports", "discourse/lib/url", "discourse/widgets/widget", "I18n", "discourse/lib/notification-levels", "discourse/lib/ajax", "discourse-common/lib/get-url", "virtual-dom", "discourse-common/lib/later", "discourse/lib/intercept-click"], function (_exports, _url, _widget, _I18n, _notificationLevels, _ajax, _getUrl, _virtualDom, _later, _interceptClick) {
  "use strict";
  ...
  0; //eaimeta@70e063a35619d71f0,"discourse/lib/url",0,"discourse/widgets/widget",0,"I18n",0,"discourse/lib/notification-levels",0,"discourse/lib/ajax",0,"discourse-common/lib/get-url",0,"virtual-dom",0,"discourse-common/lib/later",0,"discourse/lib/intercept-click"eaimeta@70e063a35619d71f
  ...
  var _default = (0, _widget.createWidget)("hamburger-menu", {
    ...
    generalLinks() {
      ...
      const canSeeUserProfiles = currentUser || !siteSettings.hide_user_profiles_from_public;
      ...
      const extraLinks = flatten((0, _widget.applyDecorators)(this, "generalLinks", attrs, state));
    ...
    listCategories() {
      ...
      let categories = [];
      if (currentUser) {
        const allCategories = site.get("categories").filter(c => c.notification_level !== _notificationLevels.NotificationLevels.MUTED);
        categories = allCategories.filter(c => c.get("newTopics") > 0 || c.get("unreadTopics") > 0).sort((a, b) => {
        ...
        categories = categories.concat(allCategories.filter(c => !categories.includes(c)).sort((a, b) => b.topic_count - a.topic_count));
      } else {
        categories = site.get("categoriesByCount").filter(c => c.notification_level !== _notificationLevels.NotificationLevels.MUTED);
      }
      if (!siteSettings.allow_uncategorized_topics) {
        categories = categories.filter(c => c.id !== site.uncategorized_category_id);
      }
    ...
    footerLinks(prioritizeFaq, faqUrl) {
      ...
      const extraLinks = flatten((0, _widget.applyDecorators)(this, "footerLinks", attrs, state));
      return links.concat(extraLinks).map(l => this.attach("link", l));
    },
    panelContents() {
      const {
      ...
      const results = [];
      const faqUrl = siteSettings.faq_url || (0, _getUrl.default)("/faq");
      ...
      if (prioritizeFaq) {
        results.push(this.attach("menu-links", {
          ...
          heading: true,
          contents: () => {
            return this.attach("priority-faq-link", {
              href: faqUrl
            });
          }
        }));
      ...
      if (currentUser && currentUser.staff) {
        results.push(this.attach("menu-links", {
          name: "admin-links",
          contents: () => {
            const extraLinks = flatten((0, _widget.applyDecorators)(this, "admin-links", attrs, state));
    ...
    refreshReviewableCount(state) {
      ...
      state.loading = true;
      return (0, _ajax.ajax)("/review/count.json").then(_ref => {
        let {
          count
        } = _ref;
    ...
    html(attrs, state) {
      if (this.currentUser && !this.currentUser.redesigned_user_menu_enabled && !state.loaded) {
    ...
    clickOutsideMobile(e) {
      ...
      const parents = document.elementsFromPoint(e.clientX, e.clientY).some(ele => ele.classList.contains("panel"));
      if (!centeredElement.classList.contains("header-cloak") && parents) {
        ...
        (0, _later.default)(() => this.sendWidgetAction("toggleHamburger"), 200);
      }
    },
    clickOutside(e) {
