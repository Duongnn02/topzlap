define("discourse/routes/app-route-map", ["exports", "discourse/models/site", "@ember/string"], function (_exports, _site, _string) {
  "use strict";
  ...
  0; //eaimeta@70e063a35619d71f0,"discourse/models/site",0,"@ember/string"eaimeta@70e063a35619d71f
  function _default() {
    // Error page
    this.route("exception", {
      path: "/exception"
    });
    this.route("exception-unknown", {
      path: "/404"
    });
    this.route("about", {
      path: "/about",
    ...
    this.route("discovery", {
      ...
      _site.default.currentProp("periods").forEach(period => {
        const top = "top" + (0, _string.capitalize)(period);
        this.route(top, {
          path: "/top/" + period
        });
      ...
      _site.default.currentProp("filters").forEach(filter => {
        this.route(filter, {
          path: "/" + filter
        });
      ...
      this.route("categoryNone", {
        path: "/c/*category_slug_path_with_id/none"
      });
      this.route("categoryAll", {
        path: "/c/*category_slug_path_with_id/all"
      });
      this.route("category", {
        path: "/c/*category_slug_path_with_id"
      });
    });
    this.route("groups", {
      resetNamespace: true,
    ...
    this.route("password-reset", {
      path: "/u/password-reset/:token"
    });
    this.route("account-created", {
    ...
    this.route("email-login", {
      path: "/session/email-login/:token"
    });
    this.route("second-factor-auth", {
      path: "/session/2fa"
    });
    this.route("associate-account", {
    ...
    this.route("tag", {
      ...
      _site.default.currentProp("filters").forEach(filter => {
        this.route("show" + (0, _string.capitalize)(filter), {
    ...
    this.route("tags", {
      ...
      _site.default.currentProp("filters").forEach(filter => {
        this.route("showCategory" + (0, _string.capitalize)(filter), {
          path: "/c/*category_slug_path_with_id/:tag_id/l/" + filter
        });
        this.route("showCategoryAll" + (0, _string.capitalize)(filter), {
          path: "/c/*category_slug_path_with_id/all/:tag_id/l/" + filter
        });
        this.route("showCategoryNone" + (0, _string.capitalize)(filter), {
      ...
      // legacy route
      this.route("legacyRedirect", {
        path: "/:tag_id"
      });
    });
    this.route("tagGroups", {
      path: "/tag_groups",
      resetNamespace: true
    }, function () {
      this.route("edit", {
