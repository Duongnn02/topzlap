define("discourse/controllers/keyboard-shortcuts-help", ["exports", "@ember/controller", "I18n", "discourse/lib/utilities", "discourse/mixins/modal-functionality", "discourse/lib/keyboard-shortcuts"], function (_exports, _controller, _I18n, _utilities, _modalFunctionality, _keyboardShortcuts) {
  "use strict";
  ...
  0; //eaimeta@70e063a35619d71f0,"@ember/controller",0,"I18n",0,"discourse/lib/utilities",0,"discourse/mixins/modal-functionality",0,"discourse/lib/keyboard-shortcuts"eaimeta@70e063a35619d71f
  const KEY = "keyboard_shortcuts_help";
  const SHIFT = _I18n.default.t("shortcut_modifier_key.shift");
  const ALT = (0, _utilities.translateModKey)("Alt");
  const META = (0, _utilities.translateModKey)("Meta");
  const CTRL = _I18n.default.t("shortcut_modifier_key.ctrl");
  const ENTER = _I18n.default.t("shortcut_modifier_key.enter");
  const COMMA = _I18n.default.t(`${KEY}.shortcut_key_delimiter_comma`);
  const PLUS = _I18n.default.t(`${KEY}.shortcut_key_delimiter_plus`);
  ...
  function buildHTML(keys1, keys2, keysDelimiter, shortcutsDelimiter) {
    const allKeys = [keys1, keys2].reject(keys => keys.length === 0).map(keys => keys.map(k => `<kbd>${k}</kbd>`).join(keysDelimiter)).map(keys => shortcutsDelimiter !== "space" ? wrapInSpan(keys) : keys);
    const [shortcut1, shortcut2] = allKeys;
    if (allKeys.length === 1) {
      return shortcut1;
    } else if (shortcutsDelimiter === "or") {
      return _I18n.default.t(`${KEY}.shortcut_delimiter_or`, {
        shortcut1,
        shortcut2
      });
    } else if (shortcutsDelimiter === "slash") {
      return _I18n.default.t(`${KEY}.shortcut_delimiter_slash`, {
        shortcut1,
        shortcut2
      });
    } else if (shortcutsDelimiter === "space") {
      return wrapInSpan(_I18n.default.t(`${KEY}.shortcut_delimiter_space`, {
        shortcut1,
        shortcut2
      }));
    }
  ...
  var _default = _controller.default.extend(_modalFunctionality.default, {
    ...
    _defineShortcuts() {
      let shortcuts = {
        jump_to: {
          shortcuts: this._buildJumpToSection()
        },
        application: {
          shortcuts: {
    ...
    _buildExtraShortcuts(shortcuts) {
      for (const [category, helps] of Object.entries(_keyboardShortcuts.extraKeyboardShortcutsHelp)) {
        helps.forEach(help => {
          ...
          shortcuts[category].shortcuts[help.name] = buildShortcut(help.name, this._transformExtraDefinition(help.definition));
        });
      }
    },
    _addCountsToShortcutCategories(shortcuts) {
      for (const [category, shortcutCategory] of Object.entries(shortcuts)) {
        shortcuts[category].count = Object.keys(shortcutCategory.shortcuts).length;
      }
    },
    _transformExtraDefinition(definition) {
      if (definition.keys1) {
        definition.keys1 = definition.keys1.map(key => this._translateKeys(key));
      }
      if (definition.keys2) {
        definition.keys2 = definition.keys2.map(key => this._translateKeys(key));
      }
      ...
      if (definition.shortcutsDelimiter) {
        definition.shortcutsDelimiter = this._translateKeys(definition.shortcutsDelimiter);
      }
      return definition;
    },
    _translateKeys(string) {
      for (const [matcher, replacement] of Object.entries(translationForExtraShortcuts)) {
